...
- 本地开发固定使用 `PORT=1000`，访问地址为 `http://localhost:1000`。
- Express 和 Vite 都不要自动切换到新端口；端口被占用时应直接失败并报告占用。
- 如果 `PORT=1000` 被占用，先确认占用进程是否是旧的 AtomFlow 服务，再停止旧进程后重新运行 `npm run dev`。
- Agent 不要为了“先跑起来”临时改 `.env`、`package.json`、`vite.config.ts` 或文档里的端口。
- 生产/云平台仍由平台注入 `PORT`，不要把本地固定端口写死到部署配置里。
...
| `WRITE_AGENT_ALLOWED_MODELS` | Optional | Comma-separated server allowlist for canvas Agent models; configured default model is always allowed |
| `WRITE_AGENT_MAX_OUTPUT_TOKENS` | Optional | Server-side maximum output tokens per canvas Agent call (default 2000) |
| `PAID_OPERATION_DAILY_LIMIT` | Optional | Shared PostgreSQL-backed daily paid-operation cap per user (default 100) |
| `PAID_OUTPUT_TOKENS_DAILY_LIMIT` | Optional | Shared daily reserved AI-token cap per user; canvas runs include estimated input tokens (default 200000) |
| `CANVAS_MAX_AGGREGATE_CONTEXT_CHARS` | Optional | Maximum combined text context for one canvas AI request (default 120000) |
| `CANVAS_AGENT_BATCH_STALE_MS` | Optional | Time before an interrupted Agent-group batch may be recovered (default 600000) |
| `CANVAS_PDF_MAX_PAGES` | Optional | Maximum PDF pages parsed from one canvas upload (default 100) |
| `AUTH_VERIFY_IP_RATE_LIMIT` | Optional | IP-wide verification attempt cap per 15 minutes (default 30) |
...
CANVAS_MAX_AGGREGATE_CONTEXT_CHARS=120000
CANVAS_AGENT_BATCH_STALE_MS=600000
