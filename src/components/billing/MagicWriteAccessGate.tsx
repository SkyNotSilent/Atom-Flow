...
export const MagicWriteAccessGate: React.FC<Props> = ({ children, onBack }) => {
  ...
  const shouldUsePortal = status.provider !== 'alipay' && status.hasBillingCustomer
    && (status.subscriptionStatus === 'paused' || status.subscriptionStatus === 'canceled' || status.paymentActionRequired || status.access === 'full');
  const canRenewAccess = status.access === 'read_only'
    && (status.provider === 'alipay' || status.subscriptionStatus === null || status.subscriptionStatus === 'canceled');
    ...
    <div className="flex h-full min-h-0 flex-col">
      {status.access === 'read_only' || status.paymentActionRequired || checkoutState.phase === 'confirming' || checkoutState.phase === 'pending' || checkoutState.phase === 'error' ? (
        <div className={`flex min-h-11 shrink-0 flex-wrap items-center justify-center gap-x-3 gap-y-1 px-4 py-2 text-center text-xs ${status.paymentActionRequired ? 'bg-[#8B3F31] text-white' : 'border-b border-[#D9C9B3] bg-[#F5E7CE] text-[#684E2E]'}`} role="status">
          <span>{checkoutState.phase === 'confirming' ? '付款已完成，正在等待服务端确认开通…' : checkoutState.phase === 'pending' || checkoutState.phase === 'error' ? checkoutState.error : status.paymentActionRequired ? '付款需要更新，完成前仍可继续使用。' : 'Pro 使用期已到期，当前为只读模式：可查看、复制与下载历史内容。'}</span>
          ...
          {canRenewAccess && billingCatalogState.phase === 'ready' ? billingPlans.filter(plan => !plan.code.startsWith('team_')).map(plan => <button key={plan.code} type="button" disabled={checkoutLocked} onClick={() => void startBillingCheckout(plan.code)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-[#255F9E] px-3 font-semibold text-white disabled:opacity-50"><CreditCard size={13} />{plan.interval === 'year' ? `购买一年 ¥${plan.priceCny}` : `购买一月 ¥${plan.priceCny}`}</button>) : null}
          {canRenewAccess && billingCatalogState.phase === 'error' ? <button type="button" onClick={() => void refreshBillingPlans()} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-[#BCA784] bg-white/80 px-3 font-semibold text-[#785A31]"><RefreshCw size={13} />重新加载套餐</button> : null}
