...
- 前端：React + Vite + TypeScript + Tailwind 风格类
- 后端：Express（同进程）+ RSS Parser + Readability + JSDOM
- 数据：当前以内存 + 本地缓存文件为主（`.cache/articles.json`）
...
优先按 `articleId` 关联原文；若旧数据缺失 `articleId`，回退到 `articleTitle` 匹配标题，避免来源视图空白。

实现位置：

- [articleDisplay.ts](file:///F:/AI产品经理/AtomFlow/src/utils/articleDisplay.ts)
...
- 增加 `docs/release-checklist.md`（发版检查清单）
- 补充 API 文档（请求/响应样例）
- 将卡片生成策略与提示词管理抽离配置化
