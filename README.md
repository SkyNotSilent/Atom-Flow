...
- **真实生产环境**：已接入 OpenAI-compatible API（MiMo Token Plan），用于知识原子化和写作助手
...
**AI 原子化（Agent 1）**
- **真实生产环境 AI**：接入 OpenAI-compatible API（MiMo Token Plan），用于知识原子化和写作助手
...
访问 `http://localhost:1000` 即可使用。本地开发端口固定为 `1000`；如果端口被占用，先停止旧进程，不要临时切换到新端口。
...
`AI_BASE_URL` 可以填带 `/v1` 的 OpenAI 兼容地址；后端会自动拼成 `/chat/completions`。MiMo 模型名会在后端自动转小写，避免误填控制台显示名导致请求失败。
